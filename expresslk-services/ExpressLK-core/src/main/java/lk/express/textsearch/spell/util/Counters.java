...
public class Counters {
	...
	/**
	 * Sets each value of target to be target[k]+arg[k] for all keys k in
	 * target.
	 */
	...
	/**
	 * Normalizes the target counter in-place, so the sum of the resulting
	 * values equals 1.
	 * 
	 * @param <E>
	 * @param target
	 ...
	 * their associated values stored in the counter. This comparator returns
	 * keys by ascending numeric value. Note that this ordering is not fixed,
	 * but depends on the mutable values stored in the Counter. Doing this
	 * comparison does not depend on the type of the key, since it uses the
	 * numeric value, which is always Comparable.
	 *
	 * @param counter
	 ...
	 * Returns a comparator backed by this counter: two objects are compared by
	 * their associated values stored in the counter. This comparator returns
	 * keys by descending numeric value. Note that this ordering is not fixed,
	 * but depends on the mutable values stored in the Counter. Doing this
	 * comparison does not depend on the type of the key, since it uses the
	 * numeric value, which is always Comparable.
	 *
	 ...
	 * by their respective value or magnitude (by absolute value). If
	 ...
	 * does, however, normalize the "to" Counter. Result is meaningless if any
	 * of the counts are negative.
	 *
	 ...
	 * counts are negative. In other words, how well can c1 be represented by
	 * c2. if there is some value in c1 that gets zero prob in c2, then return
	 * positive infinity.
	 *
	 * @param from
	 * @param to
	...
	/**
	 * Calculates the Jensen-Shannon divergence between the two counters. That
	 * is, it calculates 1/2 [KL(c1 || avg(c1,c2)) + KL(c2 || avg(c1,c2))] .
	 *
	 * @param c1
	 * @param c2
	 ...
	 * <i>Implementation note:</i> The method name favors legibility of the L
	 * over the convention of using lowercase names for methods.
	 *
	 * @param c
	 *            The Counter
	 * @return Its length
	 ...
	 * The average Counter will contain the union of keys in both source
	 * Counters, and each count will be the average of the two source counts for
	 * that key, where as usual a missing count in one Counter is treated as
	 * count 0.
	 *
	...
	private static <E> void loadIntoCounter(String filename, Class<E> c, Counter<E> counter) throws RuntimeException {
		try {
			...
			while (line != null && line.length() > 0) {
				...
				counter.setCount(m.newInstance(line.substring(0, endPos).trim()),
	...
	public static <T extends Comparable<T>> T sample(Counter<T> c, Random rand) {
		...
		if (rand == null) {
			rand = new Random();
		} else { // TODO: Seems like there should be a way to directly check if
					// T is comparable
	...
	/**
	 * Returns a counter where each element corresponds to the normalized count
	 * of the corresponding element in c raised to the given power.
	 */
