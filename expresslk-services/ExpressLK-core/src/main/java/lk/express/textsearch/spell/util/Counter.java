 ...
 * objects. It works like a Map, but with different methods for easily
 * getting/setting/incrementing counts for objects and computing various
 * functions with the counts. The Counter constructor and <tt>addAll</tt> method
 * can be used to copy another Counter's contents over.
 * <p/>
 * <i>Implementation notes:</i> You shouldn't casually add further methods to
 * this interface. Rather, they should be added to the {@link Counters} class.
 * Note that this class stores a <code>totalCount</code> field as well as the
 * map. This makes certain operations much more efficient, but means that any
 * methods that change the map must also update <code>totalCount</code>
 * appropriately. If you use the <code>setCount</code> method, then you cannot
 * go wrong. This class is not threadsafe: If multiple threads are accessing the
 * same counter, then access should be synchronized externally to this class.
 * 
...
public class Counter<E> implements Serializable, Iterable<E> {
	 ...
	 * <i>Implementation note:</i> A new Counter is allocated with its own
	 * counts, but keys will be shared and should be an immutable class.
	 *
	 * @param c
	 *            The Counter which will be copied.
	 */
	...
	/**
	 * Constructs a new Counter by counting the elements in the given
	 * Collection. The Counter is backed by a HashMap.
	 *
	 * @param collection
	 *            Each item in the Collection is made a key in the Counter with
	 *            count being its multiplicity in the Collection.
	 */
	...
	public double getCount(E key) {
		...
		if (count == null) {
			return defaultValue; // haven't seen this object before -> default
									// count
		}
		...
		return new AbstractCollection<Double>() {
			...
			public Iterator<Double> iterator() {
				return new Iterator<Double>() {
					...
					public Double next() {
						// copy so as to give safety to mutable internal
						// representation
	 ...
	 * but it's used in a number of places, and I've left it in for now. Use is
	 * discouraged.
	 *
	...
	/**
	 * This is used internally to the class for getting back a MutableDouble in
	 * a remove operation. Not for public use.
	 *
	 * @param key
	 *            The key to remove
	...
	/**
	 * Removes all the given keys from this Counter. Keys may be included that
	 * are not actually in the Counter - no action is taken in response to those
	 * keys. This behavior should be retained in future revisions of Counter
	 * (matches HashMap).
	 *
	 * @param keys
	...
	/**
	 * Equality is defined over all Counter implementations. Two Counters are
	 * equal if they have the same keys explicitly stored with the same values.
	 * <p>
	 * Note that a Counter with a key with value defaultReturnValue will not be
	 * judged equal to a Counter that is lacking that key. In order for two
	 * Counters to be correctly judged equal in such cases, you should call
	 * Counters.retainNonDefaultValues() on both Counters first.
	 *
	 * @param o
	...
	/**
	 * Returns a String representation of the Counter, as formatted by the
	 * underlying Map.
	 *
	...
	/**
	 * Converts from the format printed by the toString method back into a
	 * Counter&lt;String&gt;. The toString() doesn't escape, so this only works
	 * providing the keys of the Counter do not have commas or equals signs in
	 * them.
	 *
	 * @param s
	...
	private static class MutableDouble extends Number implements Comparable<MutableDouble> {
		 ...
		 * contains the same <code>double</code> value as this object. Note that
		 * a MutableDouble isn't and can't be equal to an Double.
		 *
		 * @param obj
		 ...
		 * @return Tthe value <code>0</code> if this <code>MutableDouble</code>
		 *         is equal to the argument <code>MutableDouble</code>; a value
		 *         less than <code>0</code> if this <code>MutableDouble</code>
		 *         is numerically less than the argument
		 *         <code>MutableDouble</code>; and a value greater than
		 *         <code>0</code> if this <code>MutableDouble</code> is
		 *         numerically greater than the argument
		 *         <code>MutableDouble</code> (signed comparison).
		 */
